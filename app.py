...
async def get_incomplete_assignments(student_id: str):
    ...
    return [{"story_title":await get_story_title(assignment['story_id']),"started": assignment['started'], "deducts_practice_rights": assignment['deducts_practice_rights'], "due_date": assignment['due_date']} for assignment in assignments]

async def get_completed_assignments(student_id: str):
    ...
    return [{"story_title":await get_story_title(assignment['story_id']), "deducts_practice_rights": assignment['deducts_practice_rights'], "due_date": assignment['due_date']} for assignment in assignments]

async def get_student_suggested_stories(student_id: str):
    suggested_stories = await mongo_db.suggested_story.find({"suggested_for_id": student_id}, {"story_id": 1, "_id": 0}).sort("_id", 1).to_list()
    return [await get_story_title(story['story_id']) for story in suggested_stories[:-1]]
...
async def main() -> None:
    ...
    remaining_rights = await get_student_remaining_rights(student['_id'])
    st.write(remaining_rights)

    incomplete_assignments = await get_incomplete_assignments(student['_id'])
    with st.expander("Incomplete Assignments"):
        st.write(incomplete_assignments)
    
    completed_assignments = await get_completed_assignments(student['_id'])
    with st.expander("Completed Assignments"):
        st.write(completed_assignments)

    suggested_stories = await get_student_suggested_stories(student['_id'])
